...
android {
  ...
  signingConfigs {
    create("release") {
      // Only configure signing when credentials are provided; otherwise the
      // release build stays unsigned so CI can still run R8 (assembleRelease).
...
val syncApk = tasks.register<Copy>("syncApkToBuildOutputs") {
